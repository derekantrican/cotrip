...
router.post('/activities', (req, res) => {
  const { trip_id, date, end_date, start_time, end_time, title, description, category, cover_image, link, sort_order } = req.body;
  if (!trip_id || !date || !title) {
    return res.status(400).json({ error: 'trip_id, date, and title are required' });
  }
  const result = run(
    'INSERT INTO activities (trip_id, date, end_date, start_time, end_time, title, description, category, cover_image, link, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
    [trip_id, date, end_date || null, start_time || null, end_time || null, title, description || null, category || 'other', cover_image || null, link || null, sort_order || 0]
  );
...
router.put('/activities/:id', (req, res) => {
  ...
  const { date, end_date, start_time, end_time, title, description, category, cover_image, link, sort_order } = req.body;
  run(
    'UPDATE activities SET date = ?, end_date = ?, start_time = ?, end_time = ?, title = ?, description = ?, category = ?, cover_image = ?, link = ?, sort_order = ? WHERE id = ?',
    [
